...
CREATE TABLE `Item` (
  ...
  `img_item` varchar(50) DEFAULT NULL,
...
(1, 'Air Force 1 Ultra Flyknit', 4999, 'Air Force 1 Ultra Flyknit', 'ну очень крутые кросы', 'Доступен', 1),
(2, 'Football Shoes', 9999, 'Football Shoes', 'крутые', 'Доступен', 1);
...
ALTER TABLE `Basket`
  ...
  ADD CONSTRAINT `basket_ibfk_2` FOREIGN KEY (`id_user`) REFERENCES `Users` (`id_user`) ON DELETE RESTRICT ON UPDATE RESTRICT;
